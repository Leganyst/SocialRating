...
@router.post("/{bonus_id}/purchase", response_model=UserBonusRead)
async def purchase_bonus_endpoint(
    bonus_id: int, user_id: int, db: AsyncSession = Depends(get_db)
):
    """
    Покупка бонуса пользователем.
    """
    try:
        user_bonus = await purchase_bonus(db, user_id=user_id, bonus_id=bonus_id)
        return user_bonus
